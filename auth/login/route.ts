...
/** Login por API (JSON ou formulário). Devolve JSON quando pedido, senão redirecciona. */
export async function POST(req: NextRequest) {
  ...
  const token = await criarToken(u.id);
  const https = httpsDoPedido(req);
  const res = querJson ? NextResponse.json({ ok: true, next: destinoSeguro, token }) : NextResponse.redirect(urlPublica(req, destinoSeguro), 303);
